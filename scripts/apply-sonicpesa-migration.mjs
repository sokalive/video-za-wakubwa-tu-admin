/**
 * Apply SonicPesa billing migration (008).
 */
...
for (const envPath of [join(__dirname, "..", ".env.production.local"), join(__dirname, "..", ".env.local")]) {
...
if (!databaseUrl) {
  console.error("Missing SUPABASE_DATABASE_URL or SUPABASE_DB_PASSWORD");
  process.exit(1);
}

const migrationPath = join(__dirname, "..", "supabase", "migrations", "008_sonicpesa_billing.sql");
...
try {
  console.log("Applying SonicPesa billing migration...");
